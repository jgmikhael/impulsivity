...
% parameters
de = [.4 1.6];           	% min and max DA levels, fluctuating around 1
...
beta = 500;              	% inverse temperature for choice rule
eta = 4;                   	% POST relative pacemaker period
...
for q = 1:length(d)
    ...
    a = normpdf(tt,th(2),1/sqrt(tlh(2)));
    b = normpdf(tt,th(1),1/sqrt(tlh(1)));
    pL(q,:) = a./(a+b);
